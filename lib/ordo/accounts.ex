defmodule Ordo.Accounts do
  ...
  ## Examples
      ...
      iex> create_tenant_user(tenant, "owner@acme.pl")
      {:ok, %User{}}
  ...
  @doc ~S"""
  Delivers an invitation email to the given user. Generates the same magic-login
  token used by `deliver_login_instructions/2`; clicking the link logs the user
  in and confirms the account.

  ## Examples

      iex> deliver_invitation(user, &url(~p"/users/log-in/#{&1}"))
      {:ok, %{to: ..., body: ...}}
  ...
  def deliver_invitation(%User{} = user, magic_link_url_fun) when is_function(magic_link_url_fun, 1) do
    ...
    UserNotifier.deliver_invitation(user, magic_link_url_fun.(encoded_token))
  end
  ...
  See `Ordo.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Ordo.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  def get_user_by_session_token(token) do
    ...
    case Repo.one(query) do
      {user, token_inserted_at} -> {Repo.preload(user, :tenant), token_inserted_at}
      nil -> nil
    end
  end
