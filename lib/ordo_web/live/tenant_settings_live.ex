defmodule OrdoWeb.TenantSettingsLive do
  ...
  defp mask(token) when is_binary(token) and byte_size(token) >= 4,
  ...
  defp input_class,
    do: "w-full border border-slate-300 bg-paper-card px-3 py-2 font-mono text-sm placeholder:text-ink-mute focus:outline-none focus:ring-2 focus:ring-label"
  ...
  def render(assigns) do
    ...
    <div class="bg-paper text-ink font-body antialiased min-h-screen">
      <header class="h-14 bg-paper-card border-b-2 border-ink flex items-center px-5 gap-4">
        ...
        <.link navigate={~p"/#{@tenant.slug}/inbox"}
      ...
      <main class="max-w-2xl mx-auto px-5 py-10 space-y-12">
        <p :if={@notice} class="font-mono text-sm text-okay border border-okay bg-paper-card px-4 py-2">{@notice}</p>

        <!-- BaseLinker -->
        <section>
          <h2 class="font-mono text-[11px] tracking-[0.2em] text-ink-mute mb-1">BASELINKER</h2>
          <p class="text-sm text-ink-soft mb-4">Token API łączy Ordo z zamówieniami, przesyłkami i dokumentami sklepu. Wygenerujesz go w panelu BaseLinkera: Moje konto → API.</p>

          <div :if={!@tenant.bl_token || @editing_token} class="bg-paper-card border border-slate-200 rounded-sm px-4 py-4">
            <div class="flex items-center gap-2 mb-3">
              <span class="font-mono text-[10px] px-2 py-0.5 border border-slate-300 text-ink-mute">BRAK TOKENU</span>
            ...
            <form phx-submit="save_baselinker" class="flex gap-2">
              <input type="password" name="token" autocomplete="off" placeholder="wklej token API…" class={input_class()} />
              <button type="submit" class="bg-ink text-white font-mono text-sm px-4 py-2 hover:bg-ink-soft focus:outline-none focus-visible:ring-2 focus-visible:ring-label">
                Zapisz token
              </button>
              <button :if={@editing_token} type="button" phx-click="cancel_token" class="text-sm text-ink-mute hover:text-ink px-2">Anuluj</button>
            </form>
            <p class="text-[12px] text-ink-mute mt-2">Token jest szyfrowany w bazie i nigdy nie jest wyświetlany ponownie.</p>
          </div>

          <div :if={@tenant.bl_token && !@editing_token} class="bg-paper-card border border-slate-200 rounded-sm px-4 py-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <span class="font-mono text-[10px] px-2 py-0.5 border border-okay text-okay">POŁĄCZONO</span>
                <span class="font-mono text-sm text-ink-mute">token {mask(@tenant.bl_token)}</span>
              </div>
              <button phx-click="change_token" class="text-sm text-ink-soft hover:text-ink border border-slate-300 px-3 py-1 hover:bg-paper">Zmień token</button>
            </div>
            <p class="text-[12px] text-ink-mute mt-2">Nowy token zapisywany jest tylko przy podaniu nowej wartości.</p>
          </div>
        </section>
        ...
        <section>
          ...
          <p class="text-sm text-ink-soft mb-4">Adresy, które Ordo monitoruje. Poczta pobierana jest z folderu INBOX co minutę.</p>

          <div :if={@mailboxes != []} class="bg-paper-card border border-slate-200 rounded-sm divide-y divide-slate-100">
            <div :for={m <- @mailboxes} class="px-4 py-3 flex items-center justify-between gap-4">
              ...
              <div class="flex items-center gap-2 shrink-0">
                ...
                <button phx-click="edit_mailbox" phx-value-id={m.id} class="text-sm text-ink-soft hover:text-ink border border-slate-300 px-3 py-1 hover:bg-paper">Edytuj</button>
                <button phx-click="delete_mailbox" phx-value-id={m.id} data-confirm={"Usunąć skrzynkę #{m.email}?"} class="text-sm text-red-700 border border-red-300 px-3 py-1 hover:bg-red-50">Usuń</button>
              </div>
            </div>
          </div>
          <div :if={@mailboxes == []} class="bg-paper-card border border-dashed border-slate-300 rounded-sm px-4 py-8 text-center">
          ...
          <div class="mt-4 bg-paper-card border border-slate-200 rounded-sm px-4 py-4">
            <p class="text-sm font-semibold mb-3">{if @mb.id, do: "Edytuj skrzynkę: #{@mb.email}", else: "Dodaj skrzynkę"}</p>
            <form phx-submit="submit_mailbox" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div class="sm:col-span-2">
                ...
                <input name="mailbox[email]" value={@mb.email} type="email" placeholder="sklep@onedaymore.pl" class={input_class()} />
              </div>
              <div>
                <label class="block text-[12px] text-ink-mute mb-1">Serwer IMAP</label>
                <input name="mailbox[imap_host]" value={@mb.imap_host} placeholder="imap.example.pl" class={input_class()} />
              </div>
              <div>
              ...
              <div>
                ...
                <input name="mailbox[username]" value={@mb.username} placeholder="pełny adres e-mail" class={input_class()} />
              </div>
              <div>
                <label class="block text-[12px] text-ink-mute mb-1">Hasło</label>
                <input name="mailbox[password]" type="password" autocomplete="off" placeholder="••••••••" class={input_class()} />
              </div>
              <div class="sm:col-span-2 flex items-center gap-3 mt-1">
                <button type="submit" class="bg-ink text-white font-mono text-sm px-4 py-2 hover:bg-ink-soft focus:outline-none focus-visible:ring-2 focus-visible:ring-label">
                  {if @mb.id, do: "Zapisz zmiany", else: "Dodaj skrzynkę"}
                </button>
                <button :if={@mb.id} type="button" phx-click="cancel_edit" class="text-sm text-ink-mute hover:text-ink px-2">Anuluj edycję</button>
              </div>
            </form>
            <p class="text-[12px] text-ink-mute mt-2">Hasło jest szyfrowane w bazie. Przy edycji zapisywane jest tylko przy podaniu nowej wartości.</p>
          </div>
        </section>
        ...
        <section :if={@tenant.demo}>
          ...
          <p class="text-sm text-ink-soft mb-4">Zarządzanie zawartością środowiska demonstracyjnego.</p>
          <div class="bg-paper-card border border-slate-200 rounded-sm divide-y divide-slate-100">
            <div class="px-4 py-3 flex items-center justify-between gap-4">
              <div>
                ...
                <p class="text-[12px] text-ink-mute mt-0.5">Wczytaj przykładowy zestaw ticketów do skrzynki.</p>
              </div>
              <button phx-click="import_mailbox" class="shrink-0 border border-slate-300 px-3 py-1.5 text-sm hover:bg-paper">Importuj</button>
            </div>
            <div class="px-4 py-3 flex items-center justify-between gap-4">
              <div>
                <p class="text-sm font-medium">Scenariusze</p>
                <p class="text-[12px] text-ink-mute mt-0.5">Wrzuć pojedynczy e-mail jak od klienta.</p>
              </div>
              <div class="flex gap-2 shrink-0">
                <button phx-click="simulate" phx-value-who="anna" class="border border-slate-300 px-3 py-1.5 text-sm hover:bg-paper">Anna</button>
                <button phx-click="simulate" phx-value-who="tomasz" class="border border-slate-300 px-3 py-1.5 text-sm hover:bg-paper">Tomasz</button>
              </div>
            </div>
            <div class="px-4 py-3 flex items-center justify-between gap-4">
              <div>
                <p class="text-sm font-medium text-red-700">Wyczyść wszystkie tickety</p>
                <p class="text-[12px] text-ink-mute mt-0.5">Usuwa tickety demo. Tej operacji nie można cofnąć.</p>
              </div>
              <button phx-click="clear_inbox" data-confirm="Wyczyścić wszystkie tickety?" class="shrink-0 border border-red-300 text-red-700 px-3 py-1.5 text-sm hover:bg-red-50">Wyczyść</button>
            </div>
          </div>
        </section>
      </main>
