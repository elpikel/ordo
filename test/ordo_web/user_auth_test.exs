defmodule OrdoWeb.UserAuthTest do
  ...
  alias Ordo.Accounts
  alias Ordo.Accounts.Scope
  alias OrdoWeb.UserAuth
  alias Phoenix.LiveView
  alias Phoenix.Socket.Broadcast

  @remember_me_cookie "_ordo_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, OrdoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to the inbox when user is already logged in", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      OrdoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      eleven_minutes_ago = :second |> DateTime.utc_now() |> DateTime.shift(minute: -11)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        OrdoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
